"""
M7 v25の的中時オッズの分布と、オッズ帯別の回収率を分析
高オッズでも的中するレースはあるか？ オッズ妙味はどこにあるか？
"""
...
# Collect all bets with market trio odds
all_bets = []  # (pb, market_odds, hit, year)
for ty in TY:
...
print(f"Total bets with odds: {len(all_bets)}")

# Analysis 1: PB<=8のみ, 三連複オッズ帯別の回収率
print(f'\n=== PB<=8: 三連複オッズ帯別の回収率 ===')
print(f'  {"オッズ帯":>12} {"R数":>6} {"的中":>5} {"的中率":>7} {"回収率":>7} {"平均配当":>8}')
print(f'  {"-"*55}')

odds_ranges = [(0,5),(5,10),(10,15),(15,20),(20,30),(30,50),(50,100),(100,500)]
for lo,hi in odds_ranges:
    sub = [(pb,o,h,y) for pb,o,h,y in all_bets if pb<=8 and lo<=o<hi]
    if not sub: continue
    tb=len(sub)*B; tr=sum(B*o for _,o,h,_ in sub if h)
    hits=sum(1 for _,_,h,_ in sub if h)
    rr=tr/tb*100; hr_=hits/len(sub)*100
    avg_pay=tr/(hits*B) if hits>0 else 0
    mk=' <<<' if rr>=100 else ''
    print(f'  {lo:>3}-{hi:>3}倍 {len(sub):>6} {hits:>5} {hr_:>6.1f}% {rr:>6.1f}% {avg_pay:>7.1f}x{mk}')

# Total
sub_all = [(pb,o,h,y) for pb,o,h,y in all_bets if pb<=8]
tb=len(sub_all)*B; tr=sum(B*o for _,o,h,_ in sub_all if h)
hits=sum(1 for _,_,h,_ in sub_all if h)
print(f'  {"全体":>12} {len(sub_all):>6} {hits:>5} {hits/len(sub_all)*100:>6.1f}% {tr/tb*100:>6.1f}%')

# Analysis 2: PBフィルタなし、オッズ帯別
print(f'\n=== フィルタなし: 三連複オッズ帯別の回収率 ===')
print(f'  {"オッズ帯":>12} {"R数":>6} {"的中":>5} {"的中率":>7} {"回収率":>7}')
print(f'  {"-"*45}')
for lo,hi in odds_ranges:
    sub = [(pb,o,h,y) for pb,o,h,y in all_bets if lo<=o<hi]
    if not sub: continue
    tb=len(sub)*B; tr=sum(B*o for _,o,h,_ in sub if h)
    hits=sum(1 for _,_,h,_ in sub if h)
    rr=tr/tb*100
    mk=' <<<' if rr>=100 else ''
    print(f'  {lo:>3}-{hi:>3}倍 {len(sub):>6} {hits:>5} {hits/len(sub)*100:>6.1f}% {rr:>6.1f}%{mk}')

# Analysis 3: PB帯 x オッズ帯のクロス分析
print(f'\n=== PB x オッズのクロス回収率 ===')
pb_ranges = [(0,4),(4,6),(6,8),(8,15)]
print(f'  {"":>8}', end='')
for lo,hi in odds_ranges[:6]:
    print(f' {lo}-{hi:>3}倍', end='')
print()
print(f'  {"-"*65}')
for plo,phi in pb_ranges:
    line = f'  PB{plo}-{phi}  '
    for olo,ohi in odds_ranges[:6]:
        sub = [(pb,o,h,y) for pb,o,h,y in all_bets if plo<=pb<phi and olo<=o<ohi]
        if len(sub) >= 10:
            tb=len(sub)*B; tr=sum(B*o for _,o,h,_ in sub if h)
            rr=tr/tb*100
            line += f' {rr:>5.0f}%'
