# -*- coding: utf-8 -*-
"""最適ベット条件探索
三連複・三連単の点数制限・EV閾値・予算配分を変えて回収率を最大化する条件を探す

探索パラメータ:
  - 三連複の最大点数: 0, 3, 5, 7, 10
  - 三連単の最大点数: 5, 10, 15, 20, 30, 50
  - EV閾値: 1.0, 1.1, 1.2, 1.3, 1.5
  - 1番人気含むフィルタ: あり/なし
  - 券種: 三連複のみ / 三連単のみ / 三連複+三連単
"""
        ...
        for rid2,idxs in rd_te.items():
            ...
            for j,idx in enumerate(idxs):
                ...
                all_bets.append({'rid':rid2,'year':test_yr,'ev':float(ev),'is_hit':int(is_hit),
                                 'payout':payout,'odds':o,'pop':int(pop_te[idx]),'combo':combo})
    return all_bets
...
for b in bets_st:
    st_by_race[b['rid']].append(b)

def simulate(trio_ev_th, st_ev_th, trio_max, st_max, pop_filter, per_bet=100):
    """条件を指定してシミュレーション"""
    ...
    for rid in all_rids:
        # 三連複候補
        tr_cands = [b for b in trio_by_race.get(rid, [])
                     if b['ev'] >= trio_ev_th and (pop_filter == 0 or b['pop'] <= pop_filter)]
        tr_cands.sort(key=lambda x: -x['ev'])
        if trio_max > 0:
            tr_cands = tr_cands[:trio_max]

        # 三連単候補
        st_cands = [b for b in st_by_race.get(rid, [])
                     if b['ev'] >= st_ev_th and (pop_filter == 0 or b['pop'] <= pop_filter)]
        st_cands.sort(key=lambda x: -x['ev'])
        if st_max > 0:
            st_cands = st_cands[:st_max]
        ...
        yr = all_cands[0]['year']
        race_invest = len(all_cands) * per_bet
        race_return = sum(b['payout'] * per_bet for b in all_cands if b['is_hit'])
        any_hit = any(b['is_hit'] for b in all_cands)
        if any_hit:
            n_hit += 1
            if race_return < race_invest:
                n_hit_but_lose += 1
        ...
        n_races += 1
        n_bets += len(all_cands)

    roi = total_return / total_invest * 100 if total_invest > 0 else 0
    profit = total_return - total_invest
    yr_rois = {}
    for yr in [2024, 2025, 2026]:
        yr_rois[yr] = yr_return[yr] / yr_invest[yr] * 100 if yr_invest[yr] > 0 else 0
    avg_bets = n_bets / n_races if n_races > 0 else 0
    hit_lose_pct = 100 * n_hit_but_lose / n_hit if n_hit > 0 else 0

    return {
        'roi': roi, 'profit': profit, 'n_races': n_races, 'n_bets': n_bets,
        'invest': total_invest, 'avg_bets': avg_bets,
        'yr_rois': yr_rois, 'n_hit': n_hit, 'hit_lose_pct': hit_lose_pct,
        'min_yr': min(yr_rois.values()) if yr_rois else 0,
    }

# === 網羅的探索 ===
print(f"\n{'='*140}")
print("条件探索: 回収率を最大化する組み合わせ")
print(f"{'='*140}")
...
# 1. 三連複のみ
print("\n--- 三連複のみ ---")
print(f"{'EV>=':>5} {'pop':>4} {'max':>4} | {'R':>5} {'bets':>6} {'avg':>5} | {'ROI':>6} {'profit':>10} | {'2024':>7} {'2025':>7} {'2026':>7} | {'hit_lose':>8} {'min_yr':>7}")
for ev_th in [1.0, 1.1, 1.2, 1.3, 1.5]:
    for pop in [1, 99]:
        for mx in [3, 5, 10, 99]:
            r = simulate(ev_th, 99, mx, 0, pop)
            if r['n_bets'] < 100: continue
            pop_s = '1fav' if pop == 1 else 'all'
            mx_s = str(mx) if mx < 99 else 'inf'
            results.append(('trio', ev_th, pop, mx, 0, r))
            yr = r['yr_rois']
            print(f"{ev_th:>5.1f} {pop_s:>4} {mx_s:>4} | {r['n_races']:>5} {r['n_bets']:>6} {r['avg_bets']:>5.1f} | {r['roi']:>5.1f}% {r['profit']:>+9,.0f} | {yr[2024]:>6.1f}% {yr[2025]:>6.1f}% {yr[2026]:>6.1f}% | {r['hit_lose_pct']:>7.1f}% {r['min_yr']:>6.1f}%")
...
for ev_th in [1.0, 1.1, 1.2, 1.3, 1.5]:
    for pop in [1, 99]:
        for mx in [3, 5, 10, 20, 30, 99]:
            r = simulate(99, ev_th, 0, mx, pop)
            if r['n_bets'] < 100: continue
            pop_s = '1fav' if pop == 1 else 'all'
            mx_s = str(mx) if mx < 99 else 'inf'
            results.append(('st', ev_th, pop, 0, mx, r))
            yr = r['yr_rois']
            print(f"{ev_th:>5.1f} {pop_s:>4} {mx_s:>4} | {r['n_races']:>5} {r['n_bets']:>6} {r['avg_bets']:>5.1f} | {r['roi']:>5.1f}% {r['profit']:>+9,.0f} | {yr[2024]:>6.1f}% {yr[2025]:>6.1f}% {yr[2026]:>6.1f}% | {r['hit_lose_pct']:>7.1f}% {r['min_yr']:>6.1f}%")

# 3. 三連複+三連単（本命）
print("\n--- 三連複+三連単 ---")
print(f"{'trEV':>5} {'stEV':>5} {'pop':>4} {'trMx':>4} {'stMx':>4} | {'R':>5} {'bets':>6} {'avg':>5} | {'ROI':>6} {'profit':>10} | {'2024':>7} {'2025':>7} {'2026':>7} | {'hit_lose':>8} {'min_yr':>7}")
for tr_ev in [1.0, 1.2, 1.3]:
    for st_ev in [1.0, 1.2, 1.3, 1.5]:
        for pop in [1]:
            for tr_mx in [3, 5]:
                for st_mx in [5, 10, 15, 20, 30]:
                    r = simulate(tr_ev, st_ev, tr_mx, st_mx, pop)
                    if r['n_bets'] < 100: continue
                    results.append(('combo', tr_ev, pop, tr_mx, st_mx, r))
                    yr = r['yr_rois']
                    print(f"{tr_ev:>5.1f} {st_ev:>5.1f} 1fav {tr_mx:>4} {st_mx:>4} | {r['n_races']:>5} {r['n_bets']:>6} {r['avg_bets']:>5.1f} | {r['roi']:>5.1f}% {r['profit']:>+9,.0f} | {yr[2024]:>6.1f}% {yr[2025]:>6.1f}% {yr[2026]:>6.1f}% | {r['hit_lose_pct']:>7.1f}% {r['min_yr']:>6.1f}%")

# === ランキング（3年とも100%超、的中負け率10%以下） ===
print(f"\n{'='*140}")
print("TOP設定（3年とも100%超 AND 的中負け率<=15% AND n>=500）")
print(f"{'='*140}")

good = [x for x in results
        if x[5]['min_yr'] > 100
        and x[5]['hit_lose_pct'] <= 15
        and x[5]['n_bets'] >= 500]
good.sort(key=lambda x: -x[5]['roi'])

print(f"{'#':>3} {'type':>6} {'trEV':>5} {'stEV':>5} {'pop':>4} {'trMx':>4} {'stMx':>4} | {'R':>5} {'bets':>6} {'avg':>5} | {'ROI':>6} {'profit':>10} | {'2024':>7} {'2025':>7} {'2026':>7} | {'hit_lose':>8}")
for i, (typ, ev1, pop, tmx, smx, r) in enumerate(good[:30]):
    yr = r['yr_rois']
    pop_s = '1fav' if pop == 1 else 'all'
    ev2 = ev1 if typ != 'combo' else smx  # workaround
    if typ == 'trio':
        print(f"{i+1:>3} {'trio':>6} {ev1:>5.1f} {'  -':>5} {pop_s:>4} {tmx:>4} {'  -':>4} | {r['n_races']:>5} {r['n_bets']:>6} {r['avg_bets']:>5.1f} | {r['roi']:>5.1f}% {r['profit']:>+9,.0f} | {yr[2024]:>6.1f}% {yr[2025]:>6.1f}% {yr[2026]:>6.1f}% | {r['hit_lose_pct']:>7.1f}%")
    elif typ == 'st':
        print(f"{i+1:>3} {'st':>6} {'  -':>5} {ev1:>5.1f} {pop_s:>4} {'  -':>4} {smx:>4} | {r['n_races']:>5} {r['n_bets']:>6} {r['avg_bets']:>5.1f} | {r['roi']:>5.1f}% {r['profit']:>+9,.0f} | {yr[2024]:>6.1f}% {yr[2025]:>6.1f}% {yr[2026]:>6.1f}% | {r['hit_lose_pct']:>7.1f}%")
    else:
        # combo: ev1=tr_ev, pop=pop, tmx=tr_max, smx=st_max
        # need st_ev from the stored tuple... reconstruct
        pass

# Re-do ranking with full info
print(f"\n{'='*140}")
print("TOP設定（再掲、全情報）")
print(f"{'='*140}")

all_configs = []
# Regenerate with explicit config
for tr_ev in [1.0, 1.2, 1.3]:
    for st_ev in [1.0, 1.2, 1.3, 1.5]:
        for pop in [1]:
            for tr_mx in [0, 3, 5]:
                for st_mx in [0, 3, 5, 10, 15, 20, 30]:
                    if tr_mx == 0 and st_mx == 0: continue
                    r = simulate(tr_ev, st_ev, tr_mx, st_mx, pop)
                    if r['n_bets'] < 300: continue
                    all_configs.append((tr_ev, st_ev, pop, tr_mx, st_mx, r))

# Filter: 3年とも100%超 AND hit_lose <= 15%
good2 = [x for x in all_configs if x[5]['min_yr'] > 100 and x[5]['hit_lose_pct'] <= 15]
good2.sort(key=lambda x: -x[5]['profit'])

print(f"{'#':>3} {'trEV':>5} {'stEV':>5} {'trMx':>4} {'stMx':>4} | {'R':>5} {'avg':>5} {'cost/R':>7} | {'ROI':>6} {'profit':>10} | {'2024':>7} {'2025':>7} {'2026':>7} | {'hit_lose':>8}")
for i, (tr_ev, st_ev, pop, tmx, smx, r) in enumerate(good2[:20]):
    yr = r['yr_rois']
    cost_per_r = r['invest'] / r['n_races'] if r['n_races'] > 0 else 0
    tmx_s = str(tmx) if tmx > 0 else '-'
    smx_s = str(smx) if smx > 0 else '-'
    print(f"{i+1:>3} {tr_ev:>5.1f} {st_ev:>5.1f} {tmx_s:>4} {smx_s:>4} | {r['n_races']:>5} {r['avg_bets']:>5.1f} {cost_per_r:>6,.0f}円 | {r['roi']:>5.1f}% {r['profit']:>+9,.0f} | {yr[2024]:>6.1f}% {yr[2025]:>6.1f}% {yr[2026]:>6.1f}% | {r['hit_lose_pct']:>7.1f}%")
