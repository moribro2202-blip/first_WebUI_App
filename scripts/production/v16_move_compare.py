...
def run(include_move, label):
        ...
        if len(hl)>=5:
            ...
            if rl:
                ...
                if winners and winners[0] in hl:
                    ...
                    oz_d=oz_cache.get(rid,{}); mkt_d=odds_mkt
                    oz_inv={h:1/oz_d[h] if h in oz_d and oz_d[h]>0 else 0 for h in hl}
    ...
    print(f"  {'EV帯':>10} | {'2024':>14} | {'2025':>14} | {'2026':>14} | {'ALL':>14}")
    print(f"  {'-'*70}")
    for elo,ehi in [(0.8,1.0),(1.0,1.2),(1.2,1.5),(1.5,5.0)]:
        parts=[]
        for yr in [2024,2025,2026,'ALL']:
            sub=[d for d in all_data if elo<=d['ev']<ehi and 2<=d['odds']<=30 and (yr=='ALL' or d['year']==yr)]
            if not sub: parts.append(f"{'':>14}"); continue
...
print("="*80)
print("move_5to3 あり vs なし（2022+WF, cyb_c含む, b=free）")
print("="*80)
run(True, "move_5to3あり（24特徴量）")
run(False, "move_5to3なし（23特徴量）")
print("\nDone!")
