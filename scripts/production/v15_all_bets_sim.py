# -*- coding: utf-8 -*-
"""v15較正モデル + Stern-Harville: 全券種シミュレーション
単勝/複勝/馬連/ワイド/三連複
"""
...
def stern_harville(p):
    """Stern-Harville: place/wide/umaren/trio確率を一括計算"""
    n = len(p)
    p2 = p**LAM2; p3 = p**LAM3
...
# === WF + 較正 + Stern-Harville → 全券種EV ===
print("\nRunning WF + all bets...", flush=True)

all_bets = {'win':[], 'place':[], 'umaren':[], 'wide':[], 'trio':[]}
...
for test_yr in [2024, 2025, 2026]:
    ...
    for rid2,idxs in rd_f.items():
        ...
        for j,idx in enumerate(idxs):
            cal_p.append(tp[j]); cal_y.append(y_f[idx])
    cal_p=np.array(cal_p); cal_y=np.array(cal_y)
    logit_p=np.log(np.clip(cal_p,1e-8,1-1e-8)/(1-np.clip(cal_p,1e-8,1-1e-8)))
    X_cal=np.column_stack([logit_p,logit_p**2])
    cal_model=LogisticRegression(C=1e6,max_iter=1000).fit(X_cal,cal_y)
    ...
    for rid2,idxs in race_data.items():
        ...
        p_cal=cal_model.predict_proba(Xc)[:,1]
        ...
        for j,idx in enumerate(idxs):
            o=odds_te[idx]
            if o<=0: continue
            is_hit=int(hns[j]==hns[wi[0]])
            payout=hjc.get('win_hjc',{}).get(str(hns[j]),0) if is_hit else 0
            all_bets['win'].append({'year':test_yr,'ev':float(p_cal[j]*o),'is_hit':is_hit,'payout':payout,'odds':o,'p':float(p_cal[j])})
        ...
        for j,idx in enumerate(idxs):
            ...
            all_bets['place'].append({'year':test_yr,'ev':float(place_p[j]*est_odds),'is_hit':is_place,'payout':payout,'odds':est_odds,'p':float(place_p[j])})

        # 馬連（上位2頭）— 的中はresult_fullで判定、オッズはconfirmed_odds
        top2_idx=np.argsort(p_cal)[::-1][:2]
        h1,h2=hns[top2_idx[0]],hns[top2_idx[1]]
        combo='-'.join(str(x) for x in sorted([h1,h2]))
        um_p=umaren_p.get(tuple(sorted([top2_idx[0],top2_idx[1]])),0)
        fp1,fp2=fps.get(h1,99),fps.get(h2,99)
        is_hit=int(set([fp1,fp2])==set([1,2]))
        hjc_um=hjc.get('umaren_hjc',{}).get(combo,0)
        payout=hjc_um if is_hit else 0
        # EVは推定オッズで計算（1/um_p * 0.775）
        est_odds=(1/um_p*0.775) if um_p>0.001 else 0
        all_bets['umaren'].append({'year':test_yr,'ev':float(um_p*est_odds),'is_hit':is_hit,'payout':payout,'odds':est_odds,'p':float(um_p)})

        # ワイド（上位2頭）
        w_p=wide_p.get(tuple(sorted([top2_idx[0],top2_idx[1]])),0)
        is_hit_w=int(fp1<=3 and fp2<=3)
        hjc_w=hjc.get('wide_hjc',{}).get(combo,0)
        payout_w=hjc_w if is_hit_w else 0
        est_odds_w=(1/w_p*0.75) if w_p>0.001 else 0
        all_bets['wide'].append({'year':test_yr,'ev':float(w_p*est_odds_w),'is_hit':is_hit_w,'payout':payout_w,'odds':est_odds_w,'p':float(w_p)})

        # 三連複（上位3頭）
        top3_idx=np.argsort(p_cal)[::-1][:3]
        h1t,h2t,h3t=hns[top3_idx[0]],hns[top3_idx[1]],hns[top3_idx[2]]
        combo3='-'.join(str(x) for x in sorted([h1t,h2t,h3t]))
        tr_p=trio_p.get(tuple(sorted([top3_idx[0],top3_idx[1],top3_idx[2]])),0)
        fps3=set([fps.get(h1t,99),fps.get(h2t,99),fps.get(h3t,99)])
        is_hit_t=int(fps3==set([1,2,3]))
        hjc_tr=hjc.get('sanrenpuku_hjc',{}).get(combo3,0)
        payout_t=hjc_tr if is_hit_t else 0
        est_odds_t=(1/tr_p*0.75) if tr_p>0.001 else 0
        all_bets['trio'].append({'year':test_yr,'ev':float(tr_p*est_odds_t),'is_hit':is_hit_t,'payout':payout_t,'odds':est_odds_t,'p':float(tr_p)})

# === 結果表示 ===
print(f"\n{'='*90}")
print("=== 全券種シミュレーション（v15較正+Stern-Harville、2024-2026）===")
print(f"{'='*90}")

for bet_type, label, ev_bands in [
    ('win', '単勝（全馬）', [(0.8,1.0),(1.0,1.1),(1.1,1.2),(1.2,1.5),(1.5,5.0)]),
    ('place', '複勝（全馬）', [(0.5,0.7),(0.7,0.8),(0.8,0.9),(0.9,1.0),(1.0,5.0)]),
    ('umaren', '馬連（上位2頭）', [(0.0,0.5),(0.5,1.0),(1.0,2.0),(2.0,5.0),(5.0,50.0)]),
    ('wide', 'ワイド（上位2頭）', [(0.0,0.5),(0.5,1.0),(1.0,1.5),(1.5,3.0),(3.0,50.0)]),
    ('trio', '三連複（上位3頭）', [(0.0,0.5),(0.5,1.0),(1.0,2.0),(2.0,5.0),(5.0,100.0)]),
]:
    data = all_bets[bet_type]
    print(f"\n■ {label} (n={len(data):,})")
    print(f"  {'EV帯':>10} | {'2024':>16} | {'2025':>16} | {'2026':>16} | {'ALL':>16}")
    print(f"  {'-'*80}")
    for elo,ehi in ev_bands:
        parts=[]
        for yr in [2024,2025,2026,'ALL']:
            sub=[d for d in data if elo<=d['ev']<ehi and (yr=='ALL' or d['year']==yr)]
            if not sub:
                parts.append(f"{'':>16}"); continue
    ...
    # キャリブレーション
    if data:
        avg_p=np.mean([d['p'] for d in data])
        avg_hit=np.mean([d['is_hit'] for d in data])
        ratio=avg_hit/avg_p if avg_p>0 else 0
        total_inv=len(data)*100
        total_pay=sum(d['payout']*100 for d in data if d['is_hit'])
        total_rec=total_pay/total_inv*100 if total_inv>0 else 0
        print(f"  全体: 予測P={avg_p:.4f} 実績={avg_hit:.4f} 比={ratio:.2f} 回収率={total_rec:.1f}%")
