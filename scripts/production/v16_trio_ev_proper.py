# -*- coding: utf-8 -*-
"""三連複: モデル予測確率 × HJC確定オッズ でEV計算
全組合せの中からEV>閾値のものを選んで回収率を検証
"""
...
# confirmed_oddsはレースごとにDBから取得（メモリ節約）
# db接続を保持
conf_db_path = DB
print(f"  conf_odds: query per race", flush=True)
...
for test_yr in [2024,2025,2026]:
    ...
    for rid2,idxs in race_data.items():
        ...
        for j,idx in enumerate(idxs):
            ...
            payout=hjc_cache.get(rid2,{}).get(hns[j],0) if is_hit else 0
            win_bets.append({'year':test_yr,'ev':float(p_raw[j]*o),'is_hit':is_hit,'payout':payout,'odds':o})

        # モデル確率のStern-Harville
        _, mdl_wide_p, mdl_umaren_p, mdl_trio_p = stern_harville(p_raw)

        # 市場確率: confirmed_oddsのプール確率（レースごとにDB取得）
        cdb = sqlite3.connect(conf_db_path, timeout=10)
        co_trio_all = {r[0]:r[1] for r in cdb.execute(
            "SELECT combination,odds FROM confirmed_odds WHERE race_id=? AND bet_type='sanrenpuku' AND odds>0",(rid2,)).fetchall()}
        co_wide_all = {r[0]:r[1] for r in cdb.execute(
            "SELECT combination,odds FROM confirmed_odds WHERE race_id=? AND bet_type='wide' AND odds>0",(rid2,)).fetchall()}
        cdb.close()

        # プール確率を計算
        def pool_probs(co_dict, total_sum):
            """オッズ逆数を正規化してプール確率を返す"""
            if not co_dict: return {}
            inv_sum = sum(1/o for o in co_dict.values() if o > 0)
            if inv_sum <= 0: return {}
            return {k: (1/o) / inv_sum * total_sum for k, o in co_dict.items() if o > 0}

        pool_trio = pool_probs(co_trio_all, 1.0)
        pool_wide = pool_probs(co_wide_all, 3.0)

        # 的中組合せの特定
        top3_actual = sorted([(fps.get(hns[i],99),i) for i in range(n)])[:3]
        if top3_actual[2][0] > 3: continue
        win_combo = tuple(sorted([top3_actual[0][1],top3_actual[1][1],top3_actual[2][1]]))
        win_combo_str = '-'.join(str(hns[i]) for i in sorted(win_combo))

        # HJC確定オッズ
        hjc_trio_payout = hjc_trio.get(rid2,{}).get(win_combo_str, 0)
        hjc_wide_payouts = hjc_wide.get(rid2,{})

        # 上位8頭の全組合せでEV計算（分母=プール確率）
        top8 = np.argsort(p_raw)[::-1][:8]
        for combo_idx in combinations(range(len(top8)), 3):
            i1,i2,i3 = top8[combo_idx[0]], top8[combo_idx[1]], top8[combo_idx[2]]
            key = tuple(sorted([i1,i2,i3]))
            mdl_p = mdl_trio_p.get(key, 0)
            if mdl_p <= 0: continue

            combo_str = '-'.join(str(hns[i]) for i in sorted(key))
            pool_p = pool_trio.get(combo_str, 0)
            if pool_p <= 0: continue

            # EV = 払戻率 × モデル確率 / プール確率
            R_trio = 0.75
            ev = R_trio * mdl_p / pool_p

            is_hit = int(key == win_combo)
            payout = hjc_trio_payout if is_hit else 0

            trio_bets.append({
                'year': test_yr, 'rid': rid2,
                'combo': combo_str,
                'trio_prob': float(mdl_p),
                'pool_prob': float(pool_p),
                'ev': float(ev),
                'is_hit': is_hit,
                'payout': payout,
            })

        # ワイド（上位6頭の全組合せ、分母=プール確率）
        top6 = np.argsort(p_raw)[::-1][:6]
        for combo_idx in combinations(range(len(top6)), 2):
            i1,i2 = top6[combo_idx[0]], top6[combo_idx[1]]
            key = tuple(sorted([i1,i2]))
            mdl_p = mdl_wide_p.get(key, 0)
            if mdl_p <= 0: continue

            combo_str = '-'.join(str(hns[i]) for i in sorted(key))
            pool_p = pool_wide.get(combo_str, 0)
            if pool_p <= 0: continue

            R_wide = 0.775
            ev = R_wide * mdl_p / pool_p

            fp1,fp2 = fps.get(hns[key[0]],99), fps.get(hns[key[1]],99)
            is_hit = int(fp1 <= 3 and fp2 <= 3)
            payout = hjc_wide_payouts.get(combo_str, 0) if is_hit else 0

            wide_bets.append({
                'year': test_yr, 'rid': rid2,
                'combo': combo_str,
                'wide_prob': float(mdl_p),
                'pool_prob': float(pool_p),
                'ev': float(ev),
                'is_hit': is_hit,
                'payout': payout,
...
print(f"\n  trio_bets: {len(trio_bets):,}")
print(f"  win_bets: {len(win_bets):,}")

def show_ev_results(data, label, ev_key='ev'):
    print(f"\n■ {label} (n={len(data):,})")
    print(f"  {'EV>=':>6} {'n':>7} {'n/年':>6} {'的中':>5} {'的中率':>7} {'回収率':>7} | {'2024':>7} {'2025':>7} {'2026':>7}")
    print(f"  {'-'*75}")
    for ev_th in [0.8, 0.9, 1.0, 1.05, 1.1, 1.15, 1.2, 1.3, 1.5]:
        sub=[d for d in data if d[ev_key]>=ev_th]
        ...
        print(f"  {ev_th:>5.2f} {n:>7} {n/3:>6.0f} {hits:>5} {hits/n:>6.2%} {rec:>6.1f}% | {' '.join(yr_recs)}")

# === EV（モデル確率/市場確率）で選択 ===
print(f"\n{'='*80}")
print("EV = モデル三連複確率 / 市場三連複確率 で選択")
print(f"{'='*80}")
show_ev_results(trio_bets, "三連複（上位8頭の全C(8,3)=56組、EV=モデルP/市場P）")
show_ev_results(wide_bets, "ワイド（上位6頭の全C(6,2)=15組、EV=モデルP/市場P）")
...
print(f"{'='*80}")
# 単勝のEVを2-30x帯に絞る
win_filtered = [b for b in win_bets if 2<=b['odds']<=30]
show_ev_results(win_filtered, "単勝（2-30x帯）")
...
for label, data, thresholds in [
    ("ワイド", wide_bets, [1.0, 1.05, 1.1, 1.15, 1.2]),
    ("三連複", trio_bets, [1.0, 1.1, 1.2, 1.3]),
]:
    print(f"\n■ {label}")
