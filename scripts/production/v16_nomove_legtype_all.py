# -*- coding: utf-8 -*-
"""move_5to3なし + 脚質バリエーション: 全馬の回収率（EVフィルタなし）"""
...
def run(use_legtype4, use_pace, label):
    ...
    for test_yr in [2024,2025,2026]:
        ...
        for rid2,idxs in race_data.items():
            ...
            if len(wi)==0: continue
            mp_te=1/(1+np.exp(-init_te[idxs]))
            mp_norm=mp_te/mp_te.sum()
            ...
            for j,idx in enumerate(idxs):
                ...
                all_data.append({'year':test_yr,'ev':float(p_raw[j]*o),'is_hit':is_hit,'payout':payout,'odds':o,'p':float(p_raw[j])})

        delta=(mkt_nll-model_nll)/n_races if n_races>0 else 0
        print(f"    {test_yr}: Delta_tau={delta:+.5f}")

    # 全馬回収率（EVフィルタなし）
    n=len(all_data); inv=n*100; pay=sum(d['payout']*100 for d in all_data if d['is_hit'])
    rec_all=pay/inv*100 if inv>0 else 0
    # 予測確率キャリブレーション
    ...
    print(f"  全馬: n={n:,} 回収率={rec_all:.1f}% 予測P={avg_p:.4f} 実績={avg_hit:.4f} 比={ratio:.2f}")

    # オッズ帯別
    print(f"  {'オッズ帯':>10} {'n':>7} {'回収率':>7} {'予測P':>7} {'実績':>6} {'比':>5}")
    for lo,hi in [(1,3),(3,5),(5,10),(10,20),(20,50),(50,999)]:
        sub=[d for d in all_data if lo<=d['odds']<hi]
        if not sub: continue
        ns=len(sub); inns=ns*100; pays=sum(d['payout']*100 for d in sub if d['is_hit'])
        recs=pays/inns*100 if inns>0 else 0
        ap=np.mean([d['p'] for d in sub]); ah=np.mean([d['is_hit'] for d in sub])
        r=ah/ap if ap>0 else 0
        print(f"  {lo:>3}-{hi:<4}x {ns:>7} {recs:>6.1f}% {ap:>6.4f} {ah:>5.4f} {r:>4.2f}")

print("="*80)
print("moveなし: 脚質バリエーション（全馬回収率 + Δ_τ + キャリブレーション）")
print("="*80)
run(False, False, "is_senkou(2値)")
run(True, False, "legtype(4値)")
run(True, True, "legtype(4値) + pace")
print("\nDone!")
