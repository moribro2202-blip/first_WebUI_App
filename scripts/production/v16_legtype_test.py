# -*- coding: utf-8 -*-
"""脚質4値化テスト: is_senkou(2値) vs legtype(4値) vs 両方"""
...
# 脚質マッピング: JRDBのrun_style → 数値
# JRDBの脚質: '逃げ','先行','差し','追込','好位差し','自在','後方'
LEGTYPE_MAP = {'逃げ': 0, '先行': 1, '差し': 2, '追込': 3, '好位差し': 2, '自在': 1, '後方': 3}

grade_map = {'G1':6,'G2':5,'G3':4,'OP':3,'L':2,'3勝':1,'2勝':0,'1勝':-1,'未勝利':-2,'新馬':-3,'一般':0}
tc_map = {'良':0,'稍重':1,'重':2,'不良':3}
sf_map = {'芝':0,'ダート':1}

def run(legtype_mode, label):
    """
    legtype_mode:
      'binary' = is_senkou(2値、現行)
      'four' = legtype(4値: 0逃/1先/2差/3追)
      'both' = is_senkou + legtype両方
    """
            ...
            if rl:
                ...
                if winners and winners[0] in hl:
                    ...
                    for i,h in enumerate(hl):
                        ...
                        if legtype_mode == 'binary':
                            f['is_senkou'] = 1 if rs in ('逃げ', '先行') else 0
                        elif legtype_mode == 'four':
                            f['legtype'] = LEGTYPE_MAP.get(rs, 2)  # デフォルト差し
                        elif legtype_mode == 'both':
                            f['is_senkou'] = 1 if rs in ('逃げ', '先行') else 0
                            f['legtype'] = LEGTYPE_MAP.get(rs, 2)
                        elif legtype_mode == 'four_pace':
                            f['legtype'] = LEGTYPE_MAP.get(rs, 2)
                            f['pace_pressure'] = n_senkou / n
    ...
    for test_yr in [2024,2025,2026]:
        ...
        for rid2,idxs in race_data.items():
            ...
            for j,idx in enumerate(idxs):
                ...
                all_data.append({'year':test_yr,'ev':float(p_raw[j]*o),'is_hit':is_hit,'payout':payout,'odds':o})

    # 結果
    print(f"\n  {label} ({len(FNAMES)}特徴量)")
    print(f"  {'EV>=':>6} {'n':>6} {'n/年':>5} {'回収率':>7} | {'2024':>7} {'2025':>7} {'2026':>7}")
    print(f"  {'-'*60}")
    for ev_th in [1.0, 1.1, 1.15, 1.2, 1.3]:
        sub=[d for d in all_data if d['ev']>=ev_th and 2<=d['odds']<=30]
        ...
        print(f"  {ev_th:>5.2f} {n:>6} {n/3:>5.0f} {rec:>6.1f}% | {' '.join(yr_recs)}")

    # top5 feature importance
    imp_model = model
    imp = dict(zip(FNAMES, imp_model.feature_importance(importance_type='gain')))
    # 脚質関連のgainだけ表示
    leg_feats = {k:v for k,v in imp.items() if 'senkou' in k or 'legtype' in k or 'pace' in k}
    if leg_feats:
        print(f"  脚質特徴量gain: {leg_feats}")

print("="*80)
print("脚質特徴量テスト（v16構成、2022+WF、move_5to3）")
print("="*80)
run('binary', 'is_senkou(2値、現行)')
run('four', 'legtype(4値: 0逃/1先/2差/3追)')
run('both', 'is_senkou + legtype 両方')
run('four_pace', 'legtype(4値) + pace_pressure')
print("\nDone!")
