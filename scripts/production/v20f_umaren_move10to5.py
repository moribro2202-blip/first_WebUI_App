# -*- coding: utf-8 -*-
"""v20f: 馬連 move_10to5 (全期間2015-2026で使える)
5分前/10分前は全年で本物のスナップショット
→ 2015-2026の11年分でWFテスト可能
"""
...
for rid,hn,odds in db.execute('SELECT race_id,horse_number,odds FROM ts_win_odds WHERE minutes_before=5 AND odds>0').fetchall():
    ts5[rid][hn] = odds
ts10 = defaultdict(dict)
for rid,hn,odds in db.execute('SELECT race_id,horse_number,odds FROM ts_win_odds WHERE minutes_before=10 AND odds>0').fetchall():
    ts10[rid][hn] = odds
...
# ペアデータセット（2015年から構築）
print("Building pair datasets (2015-2026)...", flush=True)
...
for rid, rd, vc, sf, dt in races_raw:
    ...
    # 5分前オッズを市場確率のベースに使う
    o5 = ts5.get(rid, {}); o10 = ts10.get(rid, {})
    if len(o5) < len(hl)*0.8: do_update(); continue
    has_move = len(o10) >= len(hl)*0.8

    entries = entry_cache.get(rid, {}); n = len(hl)
    # 市場確率: 5分前オッズから
    inv_arr = np.array([1/o5.get(h,999) for h in hl])
    mp = inv_arr/inv_arr.sum(); mp = mp**1.015; mp /= mp.sum()
    ...
    mk_inv = {h:1/o5.get(h,999) for h in hl}
    ...
    mkt_prob_um = {k:v/total_inv for k,v in inv_um.items()}
    ...
    # 人気順位（5分前オッズベース）
    sorted_h = sorted(o5.items(), key=lambda x:x[1])
    ...
    for h in hl:
        ...
        if has_move:
            v10 = o10.get(h,0); v5 = o5.get(h,0)
            f['move_10to5'] = (v10 - v5) / v10 if v10 > 0 and v5 > 0 else 0
        else:
            f['move_10to5'] = 0
        f['win_odds_5min'] = o5.get(h, 0)
        feats_h[h] = f
    ...
    for combo, odds_val in co.items():
        ...
        for key in ['idm_c','rider_c','total_index','expert_resid','cyb_c',
                     ...
                     'top3_rate','last_fp','win_rate','is_senkou','move_10to5']:
            v1 = f1.get(key,0); v2 = f2.get(key,0)
            pf[f'{key}_sum'] = v1 + v2; pf[f'{key}_diff'] = abs(v1 - v2)
        ow1 = f1.get('win_odds_5min',0); ow2 = f2.get('win_odds_5min',0)
        ...
        mp_val = mkt_prob_um.get(combo, 1e-8)
...
for y in sorted(pair_ds.keys()):
    ...
    print(f"  {y}: {len(d['X']):,} pairs ({n_races}R)")

# === WF（2017-2026テスト、前年でτフィット）===
...
print("\nRunning WF (test 2017-2026)...", flush=True)
all_bets = []

for test_yr in range(2017, 2027):
    train_yrs = [y for y in range(2015, test_yr) if y in pair_ds]
    fit_yr = test_yr - 1
    ...
    dtrain = lgb.Dataset(X_tr, y_tr, feature_name=PFNAMES, init_score=init_tr)
...
# === 結果 ===
print(f"\n{'='*90}")
print("v20f: 馬連 move_10to5 全期間WF")
print(f"{'='*90}")
...
# EV閾値別
print(f"\n--- EV閾値別（全期間 / 2015-2022 / 2023-2026）---")
for pop_label, pop_max in [('1番人気含む',1),('全組合せ',99)]:
    print(f"\n  {pop_label}:")
    print(f"  {'EV>=':>5} {'ALL n':>7} {'ALL rec':>7} | {'15-22 n':>7} {'15-22':>7} | {'23-26 n':>7} {'23-26':>7}")
    print(f"  {'-'*70}")
    for ev_th in [1.0, 1.1, 1.2, 1.3, 1.5]:
        sub_all = [d for d in all_bets if d['ev']>=ev_th and d['pop']<=pop_max]
        sub_old = [d for d in sub_all if d['year']<=2022]
        sub_new = [d for d in sub_all if d['year']>=2023]
        if len(sub_all)<10: continue
        def rec(data):
            if not data: return 0, 0
            n=len(data); inv=n*100; pay=sum(d['payout']*100 for d in data if d['is_hit'])
            return n, pay/inv*100 if inv>0 else 0
        n_a, r_a = rec(sub_all); n_o, r_o = rec(sub_old); n_n, r_n = rec(sub_new)
        print(f"  {ev_th:>4.1f} {n_a:>7} {r_a:>6.1f}% | {n_o:>7} {r_o:>6.1f}% | {n_n:>7} {r_n:>6.1f}%")

# 年別回収率（1番人気含む EV>=1.2）
print(f"\n--- 年別回収率（1番人気含む × EV>=1.2）---")
for yr in range(2017, 2027):
    sub = [d for d in all_bets if d['year']==yr and d['ev']>=1.2 and d['pop']<=1]
    if not sub: continue
    n=len(sub); hits=sum(d['is_hit'] for d in sub)
    inv=n*100; pay=sum(d['payout']*100 for d in sub if d['is_hit']); rec=pay/inv*100
    yr_plus = '+' if rec>100 else '-'
    print(f"  {yr}: n={n:,} 的中={hits} ({hits/n:.1%}) rec={rec:.1f}% {yr_plus}")
...
for pop_label, pop_max in [('1番人気含む',1),('全組合せ',99)]:
    for ev_th in [1.0, 1.2]:
        ...
        print(f"  {pop_label} EV>={ev_th}: n={len(sub):,} rec={pay/inv*100:.1f}% 95%CI=[{br[int(.025*len(br))]:.1f}%, {br[int(.975*len(br))]:.1f}%]")
