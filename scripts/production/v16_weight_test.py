# -*- coding: utf-8 -*-
"""頭数バイアス補正テスト: weight=1/n_horses vs なし"""
...
def run(use_weight, label):
    ...
    for rid,rd,vc,sf,dt in races_raw:
        ...
        if year not in datasets:
            datasets[year]={'X':[],'y':[],'init':[],'meta':[],'odds':[],'nhead':[]}
    ...
    for test_yr in [2024,2025,2026]:
        ...
        w_tr=None
        if use_weight:
            nhead_tr=np.concatenate([datasets[y]['nhead'] for y in train_yrs])
            w_tr = 1.0 / nhead_tr

        dtrain=lgb.Dataset(X_tr,y_tr,feature_name=FNAMES,init_score=init_tr,weight=w_tr)
        ...
        nhead_te=datasets[test_yr]['nhead']
        ...
        for rid2,idxs in race_data.items():
            ...
            for j,idx in enumerate(idxs):
                ...
                all_data.append({'year':test_yr,'ev':float(p_raw[j]*o),'is_hit':is_hit,
                                 'payout':payout,'odds':o,'nhead':int(nh)})

    # 結果
    print(f"\n  {label}")
    print(f"  {'EV>=':>6} {'n':>6} {'n/年':>5} {'回収率':>7} | {'2024':>7} {'2025':>7} {'2026':>7} | {'<12頭':>6} {'12-16':>6} {'16+':>5}")
    print(f"  {'-'*75}")
    for ev_th in [1.0, 1.1, 1.15, 1.2, 1.3]:
        ...
        # 頭数帯別
        nh_recs=[]
        for nlo,nhi in [(5,12),(12,16),(16,19)]:
            ns=[d for d in sub if nlo<=d['nhead']<nhi]
            if not ns: nh_recs.append(f"{'':>6}"); continue
            ni=len(ns)*100; np_=sum(d['payout']*100 for d in ns if d['is_hit'])
            nh_recs.append(f"{np_/ni*100:>5.1f}%")
        print(f"  {ev_th:>5.2f} {n:>6} {n/3:>5.0f} {rec:>6.1f}% | {' '.join(yr_recs)} | {' '.join(nh_recs)}")

print("="*80)
print("頭数バイアス補正テスト（v16構成、2022+WF、move_5to3）")
print("="*80)
run(False, "weight=なし（現行）")
run(True, "weight=1/n_horses（補正あり）")
print("\nDone!")
