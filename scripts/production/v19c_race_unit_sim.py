# -*- coding: utf-8 -*-
"""v19c: レース単位シミュレーション
1レースで複数馬券をかけた場合の収支をレース単位で集計
エッジ馬（モデル確率/市場確率）の組合せ × プール実オッズでEV判定
"""
...
BET_AMOUNT = 100  # 1点100円

print("\nRunning WF...", flush=True)

# レース単位の結果: {戦略名: [{year, rid, n_bets, invest, payout, has_hit}, ...]}
...
for test_yr in [2024,2025,2026]:
    ...
    for rid2, idxs in race_data.items():
        ...
        # エッジ
        edges = {hns[j]: p_model[j]/max(p_market[j],1e-8) for j in range(len(hns))}

        # SH確率
        wide_p, umaren_p, trio_p = stern_harville_full(p_model)

        # confirmed_odds
        co = defaultdict(dict)
        for row in cdb.execute("SELECT bet_type,combination,odds FROM confirmed_odds WHERE race_id=? AND odds>0 AND bet_type IN ('umaren','wide','sanrenpuku')", (rid2,)).fetchall():
        ...
        for j in range(len(hns)):
            o = mkt_odds.get(hns[j], 0)
            if not (2 <= o <= 40): continue
            ev = p_model[j] * o
            if ev >= 1.2:
                is_hit = int(hns[j] == winner)
                payout = hjc.get('win_hjc',{}).get(str(hns[j]),0) * BET_AMOUNT if is_hit else 0
                win_bets_race.append({'ev':ev,'payout':payout,'is_hit':is_hit})
        if win_bets_race:
            race_results['単勝_EV1.2'].append({
                'year':test_yr,'rid':rid2,
                'n_bets':len(win_bets_race),
                'invest':len(win_bets_race)*BET_AMOUNT,
                'payout':sum(b['payout'] for b in win_bets_race),
                'has_hit':any(b['is_hit'] for b in win_bets_race)})

        # === 各券種 × EV閾値 ===
        # エッジ馬: edge >= 1.1 の馬（上位6頭まで）
        edge_horses = sorted([h for h in hns if edges.get(h,0)>=1.1], key=lambda h:-edges[h])[:6]
        # 上位6頭（モデル順位）
        top6 = [hns[i] for i in np.argsort(p_model)[::-1][:6]]

        for ev_th_label, ev_th in [('EV1.0',1.0),('EV1.2',1.2),('EV1.5',1.5)]:
            # --- 馬連: エッジ馬×上位6頭 ---
            um_bets = []
            seen_combos = set()
            for eh in edge_horses:
                for ph in top6:
                    if eh == ph: continue
                    combo = '-'.join(str(x) for x in sorted([eh,ph]))
                    if combo in seen_combos: continue
                    seen_combos.add(combo)
                    ei,pi = hns.index(eh),hns.index(ph)
                    ...
                    fp1,fp2 = fps.get(eh,99),fps.get(ph,99)
                    is_hit = int(set([fp1,fp2])==set([1,2]))
                    payout = hjc.get('umaren_hjc',{}).get(combo,0)*BET_AMOUNT if is_hit else 0
                    um_bets.append({'ev':ev,'payout':payout,'is_hit':is_hit})
            if um_bets:
                race_results[f'馬連_{ev_th_label}'].append({
                    'year':test_yr,'rid':rid2,
                    'n_bets':len(um_bets),
                    'invest':len(um_bets)*BET_AMOUNT,
                    'payout':sum(b['payout'] for b in um_bets),
                    'has_hit':any(b['is_hit'] for b in um_bets)})
            ...
            for eh in edge_horses:
                for ph in top6:
                    ...
                    payout = hjc.get('wide_hjc',{}).get(combo,0)*BET_AMOUNT if is_hit else 0
                    w_bets.append({'ev':ev,'payout':payout,'is_hit':is_hit})
            if w_bets:
                race_results[f'ワイド_{ev_th_label}'].append({
                    'year':test_yr,'rid':rid2,
                    'n_bets':len(w_bets),
                    'invest':len(w_bets)*BET_AMOUNT,
                    'payout':sum(b['payout'] for b in w_bets),
                    'has_hit':any(b['is_hit'] for b in w_bets)})

            # --- 三連複: エッジ馬×上位6頭の3頭組 ---
            tr_bets = []
            candidates = list(set(edge_horses + top6))[:8]
            for a,b,c in combinations(candidates,3):
                ...
                payout = hjc.get('sanrenpuku_hjc',{}).get(combo,0)*BET_AMOUNT if is_hit else 0
                tr_bets.append({'ev':ev,'payout':payout,'is_hit':is_hit})
            if tr_bets:
                race_results[f'三連複_{ev_th_label}'].append({
                    'year':test_yr,'rid':rid2,
                    'n_bets':len(tr_bets),
                    'invest':len(tr_bets)*BET_AMOUNT,
                    'payout':sum(b['payout'] for b in tr_bets),
                    'has_hit':any(b['is_hit'] for b in tr_bets)})
...
# === 結果表示 ===
print(f"\n{'='*95}")
print(f"v19c: レース単位シミュレーション（1点{BET_AMOUNT}円）")
print(f"{'='*95}")

print(f"\n{'戦略':>14} {'参加R':>6} {'点/R':>5} {'的中R':>5} {'的中率':>6} {'投資':>10} {'払戻':>10} {'回収率':>7} | {'2024':>7} {'2025':>7} {'2026':>7}")
print(f"{'-'*95}")

for strat in ['単勝_EV1.2',
              '馬連_EV1.0','馬連_EV1.2','馬連_EV1.5',
              'ワイド_EV1.0','ワイド_EV1.2','ワイド_EV1.5',
              '三連複_EV1.0','三連複_EV1.2','三連複_EV1.5']:
    data = race_results.get(strat, [])
    if not data: continue
    n_races = len(data)
    avg_bets = np.mean([d['n_bets'] for d in data])
    hit_races = sum(1 for d in data if d['has_hit'])
    hit_rate = hit_races / n_races
    total_inv = sum(d['invest'] for d in data)
    total_pay = sum(d['payout'] for d in data)
    rec = total_pay / total_inv * 100 if total_inv > 0 else 0
    parts = []
    for yr in [2024,2025,2026]:
        ys = [d for d in data if d['year']==yr]
        if not ys: parts.append(f"{'':>7}"); continue
        yi = sum(d['invest'] for d in ys)
        yp = sum(d['payout'] for d in ys)
        parts.append(f"{yp/yi*100:>6.1f}%" if yi > 0 else f"{'':>7}")
    print(f"{strat:>14} {n_races:>6} {avg_bets:>4.1f} {hit_races:>5} {hit_rate:>5.1%} {total_inv:>9,} {total_pay:>9,.0f} {rec:>6.1f}% | {' '.join(parts)}")

# レース単位の収支分布
print(f"\n{'='*95}")
print("レース単位の収支分布（100円/点）")
print(f"{'='*95}")
for strat in ['単勝_EV1.2','馬連_EV1.2','ワイド_EV1.2','三連複_EV1.2']:
    data = race_results.get(strat, [])
    if not data: continue
    pnls = [d['payout'] - d['invest'] for d in data]
    pnls_arr = np.array(pnls)
    win_races = sum(1 for p in pnls if p > 0)
    print(f"\n  {strat}:")
    print(f"    参加R={len(data)} 勝ちR={win_races} ({win_races/len(data):.1%})")
    print(f"    平均投資/R={np.mean([d['invest'] for d in data]):.0f}円 平均点数/R={np.mean([d['n_bets'] for d in data]):.1f}")
    print(f"    収支: mean={np.mean(pnls):.0f}円 median={np.median(pnls):.0f}円")
    print(f"    最大利益={np.max(pnls):.0f}円 最大損失={np.min(pnls):.0f}円")
    # 分位
...
# ブートストラップCI
print(f"\n{'='*95}")
print("ブートストラップCI（レースブロック、5000回）")
print(f"{'='*95}")
for strat in ['単勝_EV1.2','馬連_EV1.0','馬連_EV1.2','ワイド_EV1.0','ワイド_EV1.2','三連複_EV1.0','三連複_EV1.2']:
    data = race_results.get(strat, [])
    if len(data) < 50: continue
    np.random.seed(42)
    br = []
    for _ in range(5000):
        samp = np.random.choice(len(data), size=len(data), replace=True)
        si = sum(data[i]['invest'] for i in samp)
        sp = sum(data[i]['payout'] for i in samp)
        if si > 0: br.append(sp/si*100)
    br.sort()
    total_inv = sum(d['invest'] for d in data)
    total_pay = sum(d['payout'] for d in data)
    rec = total_pay/total_inv*100
    print(f"  {strat:>14}: {len(data)}R rec={rec:.1f}% 95%CI=[{br[int(.025*len(br))]:.1f}%, {br[int(.975*len(br))]:.1f}%]")
