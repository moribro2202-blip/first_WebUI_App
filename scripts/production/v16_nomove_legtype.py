...
def run(include_move, use_legtype4, use_pace, label):
                ...
                if winners and winners[0] in hl:
                    ...
                    for i,h in enumerate(hl):
                        ...
                        if include_move:
                            ...
                            # ts5は外でロード済みだが、ここではglobalから取る
                            pass  # move_5to3はメインのrun関数で処理
    ...
    fb=[d for d in all_data if d['ev']>=1.2 and 2<=d['odds']<=30]
    if fb:
        n=len(fb); inv=n*100; pay=sum(d['payout']*100 for d in fb if d['is_hit'])
        print(f"  ★ EV>=1.2: n={n} 回収率={pay/inv*100:.1f}%")

print("="*80)
print("move_5to3なし + 脚質バリエーション（2022+WF）")
print("="*80)
run(False, False, False, "moveなし + is_senkou(2値)")
run(False, True, False, "moveなし + legtype(4値)")
run(False, True, True, "moveなし + legtype(4値) + pace")
print("\nDone!")
